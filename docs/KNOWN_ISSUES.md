...
| 2 | **No Google Sheets/Drive credentials.** The sync engine and Drive ingest routes are now fully coded (`/api/sync/run`, `/api/sync/status`, `/api/media/ingest` — 2026-07-17) but have **never run against a real spreadsheet or Drive folder** — there is no way to verify the column-mapping/folder-traversal logic is actually correct until dep #2 arrives. Treat as "reasoned through carefully, not proven" until then. | Volunteers cannot yet edit content without code | TASKS.md M2, M3, dep #2 |
...
| # | Issue | Impact | Plan |
|---|---|---|---|
| ~~12~~ | ~~No automated test suite~~ | ✅ **RESOLVED 2026-07-16** (Module M-B) — 45 Vitest unit tests + 17 Playwright E2E tests, 62 total. See CHANGELOG.md and TEST_REPORT.md. | — |
| 25 | **`npm run lint` fails with 9 pre-existing errors**, discovered while setting up M-B (none introduced by that module — confirmed via `git diff HEAD`, these files were untouched). All are `react-hooks/set-state-in-effect`: a widespread pattern of reading localStorage-backed demo state via `useEffect` + `setState` on mount, which a newer stricter lint rule (via `eslint-config-next` 16.2.10 / React Compiler) now flags. Affects `SaveButton.tsx`, `Header.tsx`, `Counter.tsx`, `ReportTracker.tsx`, and four admin pages (`admin/page.tsx`, `admin/reports/page.tsx`, `admin/adoptions/page.tsx`, `admin/donations/page.tsx`). Plus one `react-hooks/immutability` error in `AuthCard.tsx` (`window.location.href` assignment). | Would fail a strict CI lint gate; not a runtime bug — the app behaves correctly, this is a stricter static-analysis rule catching a legitimate-but-flaggable pattern | Flagged as a separate background task (`task_7d2df132`) rather than fixed inline — the mechanical fix (lazy `useState` initializers) touches 9 files and is out of scope for "add tests." Not yet actioned. |
