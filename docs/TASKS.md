...
|---|---|---|---|---|
| 1 | **GitHub repository** + push access | CI/CD, PR previews, code history | — | ✅ **done 2026-07-18** — owner provided `https://github.com/ShubhamSourav143/kgppaws.git`. Local `master` renamed to `main`, pushed as the initial history (67 commits' worth of work, single squashed-from-local-only history since this repo had no prior remote). **Note: Vercel's Git integration is NOT yet connected to this repo** (that requires an interactive GitHub App authorization in the Vercel dashboard, which a non-interactive session can't complete) — pushes to `main` will not auto-deploy until the owner connects Project Settings → Git in the Vercel dashboard. Deploys remain CLI-triggered (`npx vercel deploy --prod`) until then. |
