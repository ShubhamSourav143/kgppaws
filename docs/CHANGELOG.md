...
### Added
- **Official logo**: the Kharagpur Pradyogiki Animal Welfare Society seal (provided by the
  owner) is now the canonical brand mark. Source preserved at
  `public/images/branding/logo-source.jpg`; derived assets (`seal-ink.png`,
  `seal-ink-cream.png`, `seal-badge.png`) generated by the new
  `scripts/generate-brand-assets.mjs` (sharp: luminance→alpha ink extraction, recolored to
  the palette's charcoal/cream).
- `components/brand/Logo.tsx` — new `Seal` component (next/image, static imports); `Logo`
  now renders the real seal in the header, footer, and mobile drawer instead of the
  placeholder paw tile. `PawMark` retained for small illustrative uses (map markers, QR tag,
  auth card).
- About page hero now features the seal (cream ink on forest, `lg:` and up).
- **App icons regenerated from the seal**: `public/icons/icon-{192,512}.png`,
  `icon-maskable-512.png` (72% safe-zone inset), `apple-touch-icon.png`, new `app/icon.png`
  (192px favicon for modern browsers), new `app/apple-icon.png` (file convention replaces the
  manual `icons.apple` metadata field, which was suppressing file-convention icons), and a
  real multi-size `app/favicon.ico` (16/32/48 BMP entries, hand-packed in the script).
- **Social/OG card** `app/opengraph-image.png` (+ `.alt.txt`): seal badge + "Every paw has
  a story." set in the actual brand fonts (Fraunces/Manrope, rendered via headless Chromium
  at 2× and downsampled to 1200×630).

### Changed
- `public/sw.js` `VERSION` bumped v1 → v2 so returning visitors' cache-first `/icons` entries
  refresh to the new seal icons.
